...
<xsl:if test="sitemap:url/sitemap:changefreq"><th class="border-0 text-left text-truncate" scope="col">frequency</th></xsl:if>
...
<xsl:if test="xhtml:link"><a class="ml-2 text-muted" data-toggle="collapse" href="#alt-{$url_num}" role="button" aria-expanded="false" aria-controls="alt-{$url_num}">↴</a><div aria-labelledby="heading{$url_num}" class="collapse m-0 p-0" data-parent="#accordion" id="alt-{$url_num}"><xsl:apply-templates select="xhtml:link"/></div></xsl:if></td>
